(function() {
    ...
    app.controller('myPageCtrl', function($scope, $http) {
        ...
        $scope.mapInitialization = function() {
            $scope.myLatlng = new google.maps.LatLng(46.299936, 30.6558964),
                $scope.mapOptions = {
                    zoom: 17,
                    scrollwheel: false,
        ...
        $scope.contactForm = function($event) {
            ...
            $scope.target = $($event.target),
                ...
                $scope.email = $scope.target.find('[name="email"]').val(),
                $scope.tel = $scope.target.find('[name="tel"]').val(),
                $scope.subject = $scope.target.find('[name="subject"]').val(),
                $scope.message = $scope.target.find('[name="message"]').val(),
                $scope.dataString = 'name=' + $scope.name + '&email=' + $scope.email + '&tel=' + $scope.tel + '&subject=' + $scope.subject + '&message=' + $scope.message;
